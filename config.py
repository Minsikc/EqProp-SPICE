...
from utils.cost import SquaredError
from abc import ABC
...
class miniMNIST16cfg(SPICEcfg):
  ...
  def __init__(self, d:Mapping=None, hidden_dims:list=[100], **kwargs):
    ...
    self.num_classes = 5 
    self.dims = [self.INPUT_WIDTH*self.INPUT_HEIGHT*2+1]+hidden_dims+[self.num_classes*2] # input +-, bias 
    ...
    self.SPICE_params['U'] = [self.upper_frac/sqrt(n+m) for n,m in zip(self.dims, self.dims[1:])]
    self._attrcheck()
